...
#define APPEND_COMMAND(COMMAND_TYPE) { \
  COMMAND_TYPE cmd; \
  command_list_.append(cmd); \
}

#define APPEND_COMMAND_LEVEL(COMMAND_TYPE, level) { \
  COMMAND_TYPE cmd; \
  cmd.level_ = level; \
  command_list_.append(cmd); \
}
...
int PlcJob::AppendCommand(short plc_cmd_id) {
  switch (plc_cmd_id) {
    ...
    case LHC_PROGRESSIVE_THIRD:
      ...
      break;
    case LHC_LIFT:
      APPEND_COMMAND(LHC_LIFT_CMD)
      break;
    case LHC_Z_AXIS_HOLD:
      APPEND_COMMAND(LHC_Z_AXIS_HOLD_CMD)
      break;
    // Laser Command
    case LASER_ON:
      APPEND_COMMAND(LASER_ON_CMD)
      break;
    case LASER_OFF:
      APPEND_COMMAND(LASER_OFF_CMD)
      break;
    case LASER_SHUTTER_ON:
      APPEND_COMMAND(LASER_SHUTTER_ON_CMD)
      break;
    case LASER_SHUTTER_OFF:
      APPEND_COMMAND(LASER_SHUTTER_OFF_CMD)
      break;
